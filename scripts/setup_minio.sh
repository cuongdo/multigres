...
create_test_bucket() {
    ...
    if [ ! -f "${PROJECT_ROOT}/bin/mc" ]; then
        ...
        GOBIN="${PROJECT_ROOT}/bin" go install github.com/minio/mc@ee72571936f15b0e65dc8b4a231a4dd445e5ccb6
        if [ $? -ne 0 ]; then
    ...
    "${PROJECT_ROOT}/bin/mc" alias set local https://localhost:${MINIO_PORT} "${MINIO_ROOT_USER}" "${MINIO_ROOT_PASSWORD}" --insecure
    if [ $? -ne 0 ]; then
