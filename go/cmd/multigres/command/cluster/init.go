...
type initCmd struct {
	...
	minioEndpointInsecure viperutil.Value[string]
}
...
func (icmd *initCmd) buildBackupConfig(configPaths []string) (map[string]string, error) {
	...
	minioEndpoint := icmd.minioEndpointInsecure.Get()

	// Validate region requirement
	if minioEndpoint == "" && region == "" {
		return nil, fmt.Errorf("--region required for AWS S3\nExample: --backup-url=%s --region=us-east-1\nFor MinIO, use: --minio-endpoint-insecure=https://localhost:9000", backupURL)
	}
	...
	if minioEndpoint != "" {
		config["s3-endpoint"] = minioEndpoint
		config["s3-verify-tls"] = "false"

		// Print MinIO warning
		fmt.Println()
		fmt.Println("⚠ MinIO testing mode enabled (--minio-endpoint-insecure)")
		fmt.Println("  - TLS certificate verification DISABLED")
		fmt.Println("  - Using path-style requests")
		fmt.Println("  - This is for TESTING ONLY, not suitable for production")
		fmt.Println()
	}
...
func AddInitCommand(clusterCmd *cobra.Command) {
	...
	icmd := &initCmd{
		...
		minioEndpointInsecure: viperutil.Configure(reg, "minio-endpoint-insecure", viperutil.Options[string]{
			Default:  "",
			FlagName: "minio-endpoint-insecure",
			Dynamic:  false,
		}),
	}
...
backups (including AWS S3, MinIO, etc.).
...
S3 Backup Examples:
  ...
  # MinIO (testing only)
  multigres cluster init --backup-url=s3://my-bucket/backups/ \
    --minio-endpoint-insecure=https://localhost:9000`,
		RunE: icmd.runInit,
	}
	...
	cmd.Flags().String("minio-endpoint-insecure", icmd.minioEndpointInsecure.Default(),
		"MinIO/S3-compatible endpoint for TESTING ONLY (example: https://localhost:9000)\n"+
			"WARNING: Disables TLS verification. Never use in production.")

	viperutil.BindFlags(cmd.Flags(), icmd.provisioner, icmd.backupPath,
		icmd.backupURL, icmd.region, icmd.minioEndpointInsecure)
