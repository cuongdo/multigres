...
Examples:
  ...
  # Test MinIO configuration before init
  multigres cluster check-backup-config \
    --backup-url=s3://my-bucket/backups/ \
    --minio-endpoint-insecure=https://localhost:9443`,
	...
	cmd.Flags().String("minio-endpoint-insecure", ccmd.minioEndpointInsecure.Default(),
		"MinIO/S3-compatible endpoint (optional, for MinIO, etc.)")
