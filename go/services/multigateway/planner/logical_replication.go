...
func (p *Planner) planLogicalReplicationSlotCreation(sql string) (*engine.Plan, error) {
	p.logger.Debug("planning logical replication slot creation", "sql", sql)
	route := engine.NewLogicalReplicationSlotRoute(p.defaultTableGroup, constants.DefaultShard, sql)
