...
// checkMinIOAvailable checks if MinIO is available at the expected endpoint
func checkMinIOAvailable() bool {
	// Create HTTP client with TLS skip verify (for self-signed certs in dev)
	client := &http.Client{
		Timeout: 2 * time.Second,
		Transport: &http.Transport{
			TLSClientConfig: &tls.Config{InsecureSkipVerify: true}, //nolint:gosec // Test code, connecting to local MinIO
		},
	}

	// Try to connect to MinIO endpoint
	resp, err := client.Get("https://localhost:9443/minio/health/live")
	if err != nil {
		return false
	}
...
func TestCredentialRefreshIntegration(t *testing.T) {
	...
	if !checkMinIOAvailable() {
		t.Skip("MinIO is not available at https://localhost:9443 - skipping integration test")
	}
	...
	initialAccessKey := "minioadmin"
	initialSecretKey := "minioadmin"
	...
	initArgs := []string{
		...
		"--s3-bucket", "multigres",
		"--s3-region", "us-east-1",
		"--s3-endpoint", "https://localhost:9443/",
		"--s3-key-prefix", "backups/",
		"--s3-use-env-credentials",
	}
...
// TestCredentialRefreshWithoutMinIO tests refresh-credentials gracefully handles missing MinIO
func TestCredentialRefreshWithoutMinIO(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping credential refresh test in short mode")
	}

	// Setup test directory
	tempDir, err := os.MkdirTemp("/tmp", "multigres_cred_refresh_no_minio_")
	require.NoError(t, err)
	defer os.RemoveAll(tempDir)
	...
	// Initialize cluster with S3 (will fail to validate S3 if MinIO not available, but that's ok for this test)
	initArgs := []string{
		"cluster", "init",
		"--config-path", tempDir,
		"--backup-type", "s3",
		...
		"--s3-region", "us-east-1",
		"--s3-endpoint", "https://non-existent-endpoint:9999/",
		"--s3-key-prefix", "test/",
		"--s3-use-env-credentials",
	}
	...
	initCmd := exec.Command("multigres", initArgs...)
	initOutput, err := initCmd.CombinedOutput()
	// Init may fail if MinIO is not available - that's expected
	// We're testing that the refresh command works with the config even if S3 is not accessible
	if err != nil {
		t.Logf("Init failed (expected if MinIO not available): %s", string(initOutput))

		// If init failed, we can't test refresh - skip this test
		t.Skip("Cannot test refresh without successful init - MinIO likely not available")
	}

	// If we get here, init succeeded (MinIO was available)
